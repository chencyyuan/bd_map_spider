...
# citynumlist是百度地图城市代码列表，例如：深圳，北京  ，其他的可查：BaiduMap_CityCode.txt
citynumlist = ['340','131']
#需要查询什么就在列表里添加什么
keywordlist = ['酒店','医院']
start = time.time()
...
def BusinessFromBaiduDitu(citycode='340', key_word='医院', pageno=0):
    parameter = {
        "newmap": "1",
        "reqflag": "pcmap",
        "biz": "1",
    ...
    for r in htm:
        ...
        try:
            ...
            if phone[0] and '",' != phone[0]:
                ...
                number=','.join(phone_list)
                # if re.match('1', number):
                # print(citycode + name + ',' + address + ',' + number)
                writer.writerow((name, address, number))
            else:
                number="''"
                writer.writerow((name, address, number))
...
# BusinessFromBaiduDitu(citycode='340', key_word='酒店', pageno=0)
